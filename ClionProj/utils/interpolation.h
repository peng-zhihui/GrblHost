...
public:
    ...
    static double bicubicInterpolate(QRectF borderRect, QAbstractTableModel *basePoints, double x, double y)
    {
        ...
        p[0][0] = basePoints->data(basePoints->index((iy > 0 ? iy - 1 : iy), (ix > 0 ? ix - 1 : ix)),
        ...
        p[0][3] = basePoints->data(
            basePoints->index((iy > 0 ? iy - 1 : iy), (ix < basePoints->columnCount() - 2 ? ix + 2 : ix + 1)),
            Qt::UserRole).toDouble();
        ...
        p[1][3] = basePoints->data(basePoints->index(iy, ix < basePoints->columnCount() - 2 ? ix + 2 : ix + 1),
                                   Qt::UserRole).toDouble();
        ...
        p[2][3] = basePoints->data(basePoints->index(iy + 1, ix < basePoints->columnCount() - 2 ? ix + 2 : ix + 1),
                                   Qt::UserRole).toDouble();

        p[3][0] = basePoints->data(
            basePoints->index(iy < basePoints->rowCount() - 2 ? iy + 2 : iy + 1, ix > 0 ? ix - 1 : ix),
            Qt::UserRole).toDouble();
        p[3][1] = basePoints->data(basePoints->index(iy < basePoints->rowCount() - 2 ? iy + 2 : iy + 1, ix),
                                   Qt::UserRole).toDouble();
        p[3][2] = basePoints->data(basePoints->index(iy < basePoints->rowCount() - 2 ? iy + 2 : iy + 1, ix + 1),
                                   Qt::UserRole).toDouble();
        p[3][3] = basePoints->data(basePoints->index(iy < basePoints->rowCount() - 2 ? iy + 2 : iy + 1,
                                                     ix < basePoints->columnCount() - 2 ? ix + 2 : ix + 1),
                                   Qt::UserRole).toDouble();
