...
bool GcodeDrawer::updateData()
{
    switch (m_drawMode)
    {
        case GcodeDrawer::Vectors:
            if (m_indexes.isEmpty()) return prepareVectors();
            else return updateVectors();
        case GcodeDrawer::Raster:
            if (m_indexes.isEmpty()) return prepareRaster();
            else return updateRaster();
    }
}
    ...
    if (segment->drawn()) return m_colorDrawn;//QVector3D(0.85, 0.85, 0.85);
        ...
        switch (m_grayscaleCode)
        {
            case GrayscaleCode::S:
                return QColor::fromHsl(0, 0, qBound<int>(0, 255 - 255.0 / (m_grayscaleMax - m_grayscaleMin) *
                                                                  segment->getSpindleSpeed(), 255));
            case GrayscaleCode::Z:
                return QColor::fromHsl(0, 0, qBound<int>(0, 255 - 255.0 / (m_grayscaleMax - m_grayscaleMin) *
                                                                  segment->getStart().z(), 255));
        }
