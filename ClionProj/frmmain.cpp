    ...
    ui(new Ui::frmMain)
{
...
#ifndef UNIX
    ui->cboCommand->setStyleSheet(
        "QComboBox {padding: 2;} QComboBox::drop-down {width: 0; border-style: none;} QComboBox::down-arrow {image: url(noimg);	border-width: 0;}");
#endif
    ...
    connect(&m_serialPort, SIGNAL(error(QSerialPort::SerialPortError)), this,
            SLOT(onSerialPortError(QSerialPort::SerialPortError)));
...
void frmMain::preloadSettings()
{
    ...
    qApp->setStyleSheet(QString(qApp->styleSheet()).replace(QRegExp("font-size:\\s*\\d+"),
                                                            "font-size: " + set.value("fontSize", "8").toString()));
            ...
            if (m_processingFile && status != CHECK)
            {
                ...
                for (int i = m_lastDrawnLineIndex; i < list.count()
                                                   && list.at(i)->getLineNumber()
                                                      <= (m_currentModel->data(
                    m_currentModel->index(m_fileProcessedCommandIndex, 4)).toInt() + 1); i++)
                {
                    ...
                    || (m_commands[0].command == "[CTRL+X]" && dataIsReset(data)))
                {
                    ...
                    if (ca.command == "$#" && ca.tableIndex == -2)
                    {
                        ...
                        if (rx.indexIn(response) != -1)
                        {
                            ...
                            ui->cmdRestoreOrigin->setToolTip(
                                QString(tr("Restore origin:\n%1, %2, %3")).arg(m_storedX).arg(m_storedY).arg(
                                    m_storedZ));
                        }
                    }
                    ...
                    if (m_processingFile)
                    {
                        ...
                        else if (!m_fileEndSent && (m_fileCommandIndex < m_currentModel->rowCount()) &&
                                 !holding)
...
void frmMain::onSerialPortError(QSerialPort::SerialPortError error)
{
    ...
    if (error != QSerialPort::NoError && error != previousError)
    {
        ...
        ui->txtConsole->appendPlainText(
            tr("Serial port error ") + QString::number(error) + ": " + m_serialPort.errorString());
...
void frmMain::onTimerStateQuery()
{
    ...
    ui->glwVisualizer->setBufferState(
        QString(tr("Buffer: %1 / %2 / %3")).arg(bufferLength()).arg(m_commands.length()).arg(m_queue.length()));
}
...
void frmMain::onTableCurrentChanged(QModelIndex idx1, QModelIndex idx2)
{
    ...
    } else
    {
        ...
        m_selectionDrawer.setEndPosition(indexes.isEmpty() ? QVector3D(sNan, sNan, sNan) :
                                         (m_codeDrawer->getIgnoreZ() ? QVector3D(list.at(indexes.last())->getEnd().x(),
...
void frmMain::onTableDeleteLines()
{
    if (ui->tblProgram->selectionModel()->selectedRows().count() == 0 || m_processingFile ||
        QMessageBox::warning(this, this->windowTitle(), tr("Delete lines?"), QMessageBox::Yes | QMessageBox::No) ==
        QMessageBox::No)
        return;
...
void frmMain::on_actFileSave_triggered()
{
    ...
    {
        // G-code saving
        if (m_programFileName.isEmpty()) on_actFileSaveAs_triggered();
        else
        {
            ...
            && !static_cast<QKeyEvent *>(event)->isAutoRepeat())
        {
            ...
            switch (static_cast<QKeyEvent *>(event)->key())
            {
                case Qt::Key_4:
                    ...
                    break;
                case Qt::Key_6:
                    if (event->type() == QEvent::KeyPress) emit ui->cmdXPlus->pressed();
                    else emit ui->cmdXPlus->released();
                    break;
                case Qt::Key_8:
                    if (event->type() == QEvent::KeyPress) emit ui->cmdYPlus->pressed();
                    else emit ui->cmdYPlus->released();
                    break;
                case Qt::Key_2:
                    if (event->type() == QEvent::KeyPress) emit ui->cmdYMinus->pressed();
                    else emit ui->cmdYMinus->released();
                    break;
                case Qt::Key_9:
                    if (event->type() == QEvent::KeyPress) emit ui->cmdZPlus->pressed();
                    else emit ui->cmdZPlus->released();
                    break;
                case Qt::Key_3:
                    if (event->type() == QEvent::KeyPress) emit ui->cmdZMinus->pressed();
                    else emit ui->cmdZMinus->released();
                    break;
            }
        }
            ...
            case QEvent::MouseButtonDblClick:
                ...
                if (ui->grpConsole->height() == minHeight || !ui->scrollArea->verticalScrollBar()->isVisible())
                {
                    ui->splitPanels->setSizes(QList<int>() << ui->scrollArea->minimumHeight()
                                                           << ui->splitPanels->height() -
                                                              ui->splitPanels->handleWidth() -
                                                              ui->scrollArea->minimumHeight());
                } else
                {
...
{
    return key == Qt::Key_4 || key == Qt::Key_6 || key == Qt::Key_8 || key == Qt::Key_2 || key == Qt::Key_9 ||
           key == Qt::Key_3;
}
...
bool frmMain::updateHeightMapGrid()
{
    ...
    if (!nan &&
        QMessageBox::warning(this, this->windowTitle(), tr("Changing grid settings will reset probe data. Continue?"),
                             QMessageBox::Yes | QMessageBox::No) == QMessageBox::No)
        return false;
...
bool frmMain::saveHeightMap(QString fileName)
{
    ...
    for (int i = 0; i < m_heightMapModel.rowCount(); i++)
    {
        for (int j = 0; j < m_heightMapModel.columnCount(); j++)
        {
            textStream << m_heightMapModel.data(m_heightMapModel.index(i, j), Qt::UserRole).toString()
                       << ((j == m_heightMapModel.columnCount() - 1) ? "" : ";");
        }
        textStream << "\r\n";
    }
                ...
                for (int i = 0; i < m_programModel.rowCount() - 1; i++)
                {
                    ...
                    } else
                    {
                        ...
                        // Parse command args
                            ...
                            {                   // arg examples: G1, G2, M3, X100...
                                codeChar = arg.at(0).toLatin1();            // codeChar: G, M, X...
                                if (!coords.contains(codeChar))
                                {           // Not parameter
                                    codeNum = arg.mid(1).toDouble();
                                    if (g.contains(codeChar))
                                    {             // 'G'-command
                                        // Store 'G0' & 'G1'
                        ...
                        for (int j = lastSegmentIndex; j < list->count(); j++)
                        {
                            if (list->at(j)->getLineNumber() == line)
                            {
                                if (!qIsNaN(list->at(j)->getEnd().length()) &&
                                    (isLinearMove || (!hasCommand && !lastCode.isEmpty())))
                                {
                                    ...
                                    while ((j < list->count()) && (list->at(j)->getLineNumber() == line))
                                    {
                                        ...
                                        item.command = newCommand + QString("X%1Y%2Z%3")
                                            .arg(point.x(), 0, 'f', 3).arg(point.y(), 0, 'f', 3).arg(point.z(), 0, 'f',
                                                                                                     3);
                                        m_programHeightmapModel.data().append(item);
